...
Neste projeto abordou-se a criação do \emph{netlist} logo através do esquemático, as
diferenças entre a simulação de um \emph{netlist} obtido através do esquemático e de
um \emph{netlist} obtido através do \emph{layout}, assim como a comparação \emph{layout}/esquemático
através da ferramenta \emph{LVS}. Por fim introduziu-se a simulação de Monte Carlo que
tem por finalidade gerar uma análise estatística do comportamento do circuito frente a
...
\begin{figure}[!hb]
    \begin{center}
        ...
        \end{tikzpicture}
    \end{center}
    \caption{\label{figPortaLogicaQ1}Porta lógica \emph{CMOS}. Implementa a função $\overline{AB + C}$.}
\end{figure}
...
\textbf{6.} *Como são calculadas as áreas e perímetros de dreno e \emph{source} no
   ...
   \begin{eqnarray}
        ...
        PD & = & W + 2 \cdot XA,
   \end{eqnarray}
   onde $AS$ é a área de \emph{source}, $AD$ é a área de dreno, $PS$ é o perímetro
...
\textbf{8.} *Apresente os gráficos da questão anterior e copie os comandos de medida
   e sinais de entrada que usou no \emph{ELDO}.
    ...
    As formas de onda dos sinais de entrada $A$, $B$ e $C$ e a saída $S$ obtida pela 
    simulação estão na imagem \ref{imgSinEntQ8}.

    Quando mantemos o sinal $A$ em nível lógico alto e o sinal $C$ em nível lógico baixo, o circuito
    lógico da figura \ref{figPortaLogicaQ1} funciona como uma porta inversora com entrada em $B$ e 
    saída $S$. Pode-se observar na figura \ref{imgSinEntQ8} que, como esperado, a saída é a negação 
    de $C$.

    \begin{lstlisting}[caption=Comandos utilizados para a medição de atraso de subida e descida por
                       capacitância de carga., label=lstMeasureQ8]
*
* .CONNECT statements
*
.CONNECT GROUND 0
...
\textbf{11.} *Como se pode acrescentar aos \emph{ports} $V_{DD}$ e $V_{SS}$ as
   ...
   mesma \emph{layer} se comportam como um único \emph{shape}. Logo, conectar algo em um
   \emph{MET1} sobre o \emph{source} de um transistor, é o mesmo que conectar diretamente
   ao \emph{source} em questão.
...
\textbf{16.} *Uma vez feitas as verificações com \emph{DRC} e \emph{LVS}, caso não
   ...
   sinais de entrada utilizados e saída obtida pela simulação estão na figura 
...
\textbf{18.} *Comente as diferenças entre os resultados encontrados nas questões 8 e
   16/17? Dê as razões para elas.
    ...
    A simulação executada sobre o \emph{netlist} extraído do \emph{layout} demonstrou
    menores atrasos, frente aos atrasos obtidos na simulação do \emph{netlist} extraído
    do esquemático. Isso se deve ao fato das junções entre os transistores no \emph{layout}
    reduzir áreas e perímetros de source e dreno dos transistore, proporcionando melhores
    resultados.
...
\textbf{22.} *Desenhe os gráficos da questão anterior e copie os comandos de medida
   e sinais de entrada que usou no \emph{ELDO}.
    ...
    O código utilizado para a simulação está descrito no código fonte \ref{lstPniQ22}
...
\textbf{24.} *Termine \emph{layout} da célula, passe o \emph{DRC} e faça o \emph{LVS}.
   Gere uma figura do \emph{layout} mostrando todos os níveis e inclua no trabalho.

    O \emph{layout} da porta lógica não inversora está na figura \ref{imgLayoutQ24}.

    \img{\emph{Layout} da porta lógica não inversora.}{imgLayoutQ24}{layout_q24.png}
...
\textbf{25.} *Agora extraia o circuito de simulação a partir do \emph{layout} (opção
   ...
   com os resultados.
    ...
    Vale observar que, com a utilização do inversor na saída da porta lógica, ao mantermos o
    sinal $A$ em alto e $C$ em baixo obtemos uma porta repetidora com entrada $C$ e saída $S$.
    Pode-se ver na figura \ref{imgSinEntQ25} que realmente o sinal de saída acompanha os níveis
    de tensão do sinal de entrada.

    \img{Atraso alto-alto e baixo-baixo para a porta lógica não inversora obtidos através de 
        simulação do circuito com capacitâncias parasitas.}
        {imgApcQ25}
        {atraso_por_capacitancia_q25.jpg}

    \img{Sinais de entrada utilizados na simulação e saída obtida para a porta lógica não inversora}
        {imgSinEntQ25}
        {sinais_entrada_saida_q25.jpg}

    \begin{table}[!hp]
        \caption{\label{tblApcQ25}Atrasos obtidos para diferentes capacitâncias de carga através da simulação
...
\textbf{26.} *Para as curvas tempo de propagação na subida e descida geradas a
   ...
   eixo Y (eixo de tempo).
    ...
    \end{equation}
    onde $\alpha_A$ e $\alpha_B$ são os coeficientes angulares dos atrasos alto-alto 
    ...
    \end{equation}
    Substituindo um ponto na fórmula, podemos obter $y_{0A}$:
    \begin{equation}
    ...
    \begin{table}[!hp]
            \caption{Coeficientes angular e linear para as curvas de atraso de descida e subida
                por capacitância de carga para a porta lógica não inversora.}
            \label{tabCalQ17}
            \centering
        \begin{tabular}{|l|r|r|}
                                                                                            \hline 
...
\end{lstlisting}
    ...
    Os histogramas de ocorrências de atrasos estão nas figuras \ref{imgHisAltoQ28} e \ref{imgHisBaixoQ28}.
    Na figura \ref{imgSinEntQ28} podemos observar o sinal de entrada e o comportamento da saída típica, 
    melhor e pior casos.

    \img{Histograma de ocorrências de atraso alto-alto obtido pela simulação de Monte Carlo sobre a porta
        lógica não inversora.}
        {imgHisAltoQ28}
        {histograma_atraso_alto_q28.jpg}

    \img{Histograma de ocorrências de atraso baixo-baixo obtido pela simulação de Monte Carlo sobre a porta
        lógica não inversora.}
        {imgHisBaixoQ28}
        {histograma_atraso_baixo_q28.jpg}

    \img{Sinal de entrada $C$ utilizado durante a simulação de Monte Carlo sobre a porta lógica não inversora
        e sinal de saída $OUT$ no melhor caso, caso típico e pior caso obtidos pelo simulação.}
        {imgSinEntQ28}
