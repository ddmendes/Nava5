...
\textbf{2.} *Mostre que para uma corrente igual a $I_{Dlim}$ os valores de $g_m$ calculados considerando o transistor em fraca ou forte inversão coincidem.
    ...
    Utilizando esse resultado com o obtido no exercício anterior (equação \ref{eqnQ1resultado}), tem-se
        \begin{eqnarray}
            g_m & = & \frac{W}{L} \codt \frac{I_{D0}}{V_T} \cdot e^{ \frac{ V_T \cdot ln \left( \frac{I_D}{(W/L) \cdot I_{D0}} \right) }{V_T} } \\
            \Rightarrow g_m & = & \frac{W}{L} \codt \frac{I_{D0}}{V_T} \cdot \left( \frac{I_D}{(W/L) \cdot I_{D0}} \right) } \\
            \Rightarrow g_m & = & \frac{I_D}{V_T}
        \end{eqnarray}
    Considerando a corrente como $I_{Dlim}$, tem-se
        \begin{eqnarray}
            I_{D} = & I_Dlim & = & \mu \cdot C_{ox} \cdot \frac{W}{L} \cdot 2 \cdot V_T^2 \\
...
O circuito deve funcionar para tensões na saída (dreno de $M_5$) tão altas quanto ($V_{DD} - 0,4 V$). Considere que $M_3$, $M_4$ e $M_5$ estão em forte inversão.
    ...
    \end{eqnarray}
    Sabendo que $(W/L)_1 = (W/L)_2$ devido a $N = 1$, e que a corrente no dreno dos transistores equivale a $\frac{I_S}{X}$,
    \begin{equation}
        \frac{I_S / X}{I_{Dlim}} < \frac{1}{10}
    \end{equation}
    Sendo $I_{Dlim} = \mu \cdot C_{ox} \cdot \left( \frac{W}{L} \right) \cdot 2 \cdot \left( n \cdot V_T \right)^2$
    \begin{eqnarray}
        \frac{I_S}{X} & < & \frac{ \mu_P \cdot C_{ox} \cdot (W/L)_2 \cdot 2 \cdot \left(n \cdot V_T\right)^2 }{10}
        \Rightarrow (W/L)_2 & > & \frac{I_S \cdot 5}{X \cdot \mu_P \cdot C_{ox} \cdot \left(n \cdot V_T \right)^2}
    \end{eqnarray}
    ...
    \begin{eqnarray}
        I_S & = & \mu \cdot C_{ox} \cdot (W/L)_5 \cdot V_{GS}^2 \\
        \Rightarrow \left( \frac{W}{L} \right)_5 & = & \frac{I_S}{\mu \cdot C_{ox} \cdot V_{GS}^2} \\
        \Rightarrow \left( \frac{W}{L} \right)_5 & > & \frac{I_S}{\mu \cdot C_{ox} \cdot 0,16} \\
