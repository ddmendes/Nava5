...
\end{lstlisting}
    ...
    %\img{Relações entre correntes de $M_4$ e $M_3$.}{imagQ07relacaoEntreCorrentes} % XXX
...
\textbf{8.} *Ajuste o valor de $R$ para que a corrente de saída em $V_{DD} = 3,0 V$ seja a desejada no projeto (valor nominal). Apresente então o gráfico $I_S x V_{DD}$.
    ...
    %\img{Corrente X Resistência.}{imagQ08varreduraDaResistencia} % XXX
    ...
    %\img{Corrente $$I_S X Tensão $V_{DD}$.}{imagQ08correntePorTensao} % XXX
...
>>>>>>> 75b1bb7f6bb4e03fad5e90d5895b7ab73cd3faf3
...
dado que o valor ideal seria $I_S = 1,95 \mu A$.
    ...
    %\img{Faixa de tensão adequada.}{imagQ09faixaDeTensao} % XXX
...
\textbf{11.} *Reprojetar o circuito com modificações para reduzir a sua sensibilidade a variações de $V_{DD}$. Tomar cuidado para que as dimensões não aumentem muito e que a faixa de operação não seja muito reduzida. Apresente o esquemático do circuito, com as dimensões escolhidas, e o novo gráfico $I_S x V_{DD}$.
    ...
    %\img{Circuito reprojetado}{imagQ11novoCircuito} % TODO
    ...
    %\img{Corrente $I_S$ X Tensão $V_{DD}$}{imagQ11correntePorTensao} % TODO
