...
\section*{Introdução}
Nesta pratica foi realizado um trabalho extensivo com fontes geradoras de corrente de referência e de tensão de referência. Para tanto utilizou-se transistores com um grande comprimento de canal ($24 \mu m$, neste caso), o que necessita de um contato de poço ao redor de todo o transistor, garantindo uma melhor tensão em toda sua extenção. Neste projeto também cuidou-se de técnicas para garantir uma corrente confiável para uma certa variação na tensão $V_{SD}$ (tensão \emph{source-dreno}) assim como garantir uma tensão de referência confiável para uma faixa de temperatura.
...
\textbf{3.} *Considere os dois espelhos de corrente apresentdos na Figura \ref{imagQ03espelhos}. Um deles é um espelho de convencional e o outro é um espelho de corrente de Wilson. \\
    ...
    \img{Modelo de pequenos sinais de uma fonte de corrente convencional.}
        {imagQ3peqSinaisEspelho}
        {imagQ3peqSinaisEspelho}

    Observando o modelo de pequenos sinais da figura \ref{imagQ3peqSinaisEspelho}, podemos ver que $g_{02}$ é a única impedância alcançável pela saída. Logo a impedância de saída desta fonte é $g_{02}$.
    ...
    \img{Modelo de pequenos sinais do espelho de corrente Wilson}
        {imagQ3peqSinaisWilson}
        {imagQ3peqSinaisWilson}

    Do circuito descrito na figura \ref{imagQ3peqSinaisWilson}, podemos tirar:

    \begin{eqnarray}
        I_0 & = & g_{m3} \cdot (V_y - V_x) \cdot + g_{03} \cdot (V_0 - V_x) \label{q3formI0} \\
        V_x = \frac{g_{03} \cdot V_0}{g_{02} + g_{m2} + g_{03} + g_{m3} + \frac{g_{m1} \cdot g_{m3}}{g_{01}}} \label{q3formVx}
    \end{eqnarray}
    
    Substituindo (\ref{q3formVx}) em (\ref{q3formI0}) e manipulando a equação, obtemos:

    \begin{equation}
        Z_{out} = \frac{V_0}{I_0} = \frac{g_{01} \cdot (g_{02} + g_{m2} + g_{03} + g_{m3}) + g_{m3} \cdot g_{m1}}{g_{m2} \cdot g_{01} \cdot g_{03} + g_{01} \cdot g_{02} \cdot g_{03}} \label{q3formZout1}
    \end{equation}

    Considerando $g_m \gg g_0$, podemos simplicar a equação (\ref{q3formZout1}) para:

    \begin{equation}
        Z_{out} = \frac{g_{m3} \cdot g_{m1}}{g_{m2} \cdot g_{01} \cdot g_{03}}
    \end{equation}

    Como os transistores $M1$ e $M2$ são iguais, então ainda é possível reduzir para:

    \begin{equation}
        Z_{out} = \frac{g_{m3}}{g_{01} \cdot g_{03}}
    \end{equation}

    Como já utilizado, $g_m \gg g_0$, desta forma, temos que a impedância de saída do espelho de Wilson é muito maior do que a do espelho convencional. Por outro lado, como utiliza-se transistores em série, a tensão necessário para o funcionamento do circuito é maior.
...
Mostre que a corrente de saída tem, quando os transistores $M_3$, $M_4$ e $M_5$ estão em saturação, a expressão
    ...
    \img{Circuito gerador de corrente de referência.}{imagQ04gerCorrenteRef}{imagQ04getCorrenteRef}
...
Obs.: O extrator gera a linha do resistor erradamente. O resistor deve ser um subcircuito. Acrescente X no inicio da linha gerada para o resistor. Adicionalmente deve ser acrescentado ao arquivo de simulação o modelo do resistor que se encontra em /local/tools/dkit/ams\_3.70/c35/eldo/restm.mod.
    ...
    Os resultados obtidos através da simulação Monte Carlo estão respresentados a Figura \ref{imagQ16histMonteCarlo}, onde o valor médio é $1,98 \mu m$.
...
\end{verbatim}
    ...
    O código fonte \ref{cfQ20simulacao} mostra como foi realizada a simulação.

\begin{lstlisting}[caption=Simulação AC de 1\,0 KHz a 100 MHz, label=cfQ20simulacao]
...
Nela podemos ver que:
  ...
  -- o valor de $R_2$ serve para ajustar a relação entre essas duas tensões.
    ...
    \img{Tensão $V_{BE}$ x Temperatura}{imagQ24tensaoTemperatura}{imagQ24tensaoTemperatura}
...
\textbf{25.} *Projete uma fonte de tensão de referencia similar a da Figura \ref{imagQ24fonteTensaoReferencia}, mas utilize a fonte de corrente que você projetou (questão 15). Na fonte de tensão faça com que a corrente do bipolar seja igual à corrente que passa pelo resistor $R_1$ (Figura \ref{imagQ24fonteTensaoReferencia}). O valor de $R_2$ deve ser ajustado para que Coeficiente de Temperatura\footnote{Coeficiente de Temperatura: parte por milhão por grau Celsius} seja inferior a 50 ppm/ºC, para a temperaturas variando entre -10ºC e 100ºC. Apresente o esquemático do circuito completo, as dimensões dos transistores e os valores dos resistores. Apresente também o gráfico $V_{REF}$ x Temperatura. \\
    ...
    \begin{equation}
        Coeficiente de Temperatura = \frac{V_{MAX} - V_{MIN}}{V_{REF}} \cdot \frac{1}{100 - \left(-10\right)} \cdot 10^6
    \end{equation}

Obs.: Caso a tensão de saída esteja variando muito com a temperatura, reajuste o valor de $R_2$.

    O esquemático está representado na Figura \ref{imagQ25esquematico}. \\
    As dimensões são do transistores PMOS são $W = 19,8 \mu m$ e $L = 24 \mu m$ (dividido em 2 de $L = 12 \mu m$) e as do NMOS são $W = 91,1 \mu m$ e $L = 1 \mu m$. \\
...
Obs.: o transistor bipolar extraído vem com o parâmetro Area. Apague este parâmetro senão ficará errado.
    ...
    O \emph{layout} da fonte de tensão completa está representado na Figura \ref{imagQ26layoutFonte}. O gráfico de coeficiente de temperatura por comprimento $L$ do resistor está na figura \ref{imagQ26coeficienteDeTemperatura}.

    \img{\emph{Layout} da fonte de tensão completa}{imagQ26layoutFonte}{imagQ26layoutFonte}

    \img{Curva do coeficiente de temperatura por comprimento $L$ do resistor.}{imagQ26coeficienteDeTemperatura}{imagQ26coeficienteDeTemperatura}


\textbf{27.} *Adicione ao \emph{layout Pads} de $V_{DD}$ e $GND$. Passar o DRC para verificar se tudo está correto. Quais são as dimensões do circuito com os \emph{Pads}? Apresente o \emph{layout} do circuito e o gráfico $V_{REF}$ x Temperatura para valores de $V_{DD}$ de 2,0 V, 2,5 V e 3,0V. \\
Obs.: um bloco de \emph{Pad} pode ser encontrado na biblioteca IOLIB\_4M, célula g-padonly.

    O \emph{layout} do circuito está representado na Figura \ref{imagQ27layoutCircuito}, com dimensões de $114,581 \mu m$ por $325,850 \mu m$, o que resulta em uma área do circuito de $37.336,2 \mu m^2$. % Pontos (81.307, -157.450), (195.825, 168.400) \\
    O gráfico $V_{REF}$ x Temperatura para valores de $V_{DD}$ de 2,0 V, 2,5 V e 3,0V estão representados na Figura \ref{imagQ27refTemp}.

    \img[0.45]{\emph{Layout} do circuito com \emph{Pads}}{imagQ27layoutCircuito}{imagQ27layoutCircuito}
    \img{$V_{REF}$ x Temperatura}{imagQ27refTemp}{imagQ27refTemp}
