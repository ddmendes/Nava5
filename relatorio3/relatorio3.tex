...
\textbf{5.} *A partir das informações dos blocos que compõe este circuito estime o valor máximo da frequência do \emph{clock} que o circuito pode suportar (considere a entrada $D = "1"$, utilize o pior caso entre subida e descida das portas e considere que os sinais são rápidos). Apresente os cálculos. \\
    ...
    \end{equation}
    Por analisar as configurações dos componentes, sabe-se que a capacitância de carga dos elementos são $C_{NAND} = 5 fF$ e $C_{FF} = 8 fF$. Utilizar uma aproximação linear em conjunção as informações fornecidas pelo documento que contém informações sobre as células, pode-se definir $t_{set-up} = 0 ns$, $t_{hold} = 0,538 ns$ e $t_{NAND} = 0,077 ns$. % TODO checar os dados
    Assim sendo, $t_{min} = 0,615 ns$ o que implica uma frequência máxima de \emph{clock} de $1,63 GHz$. \\ % TODO terminar XXX o de relatórios anteriores deu 1,42GHz
...
\begin{table}[!h]
    \caption{Máximas frequências de operação para o divisor de frequência da \fref{imgQ04esquematico}. \label{tblQ10maxFreq}}
    \centering
    \begin{tabular}{|l|c|}
                                                        \hline
...
% Pontos {-1.7, 2.8} {46.3, 17.0} W=48um L=14.2um AREA=681.6umm
