...
\textbf{5.} *A partir das informações dos blocos que compõe este circuito estime o valor máximo da frequência do \emph{clock} que o circuito pode suportar (considere a entrada $D = "1"$, utilize o pior caso entre subida e descida das portas e considere que os sinais são rápidos). Apresente os cálculos. \\
    ...
    \begin{equation}
        \frac{1,66 - 0,61}{960 - 3} = \fract{t_{hold} - 0,61}{13 - 3}
    \end{equation}
    \begin{equation}
        \frac{1,64 - 0,07}{640 - 2} = \fract{t_{NAND} - 0,07}{5 - 2}
    \end{equation}
...
\textbf{14.} *A partir dos dados dos blocos que compõe o divisor 4/5 estime o máximo \emph{clock} que o circuito pode suportar (considere que são usados na implementação os blocos \textbf{DF1}, \textbf{NAND23} e \textbf{NOR23} e utilize o pior caso entre subida e descida das portas). Apresente os cálculos. \\
    ...
    \begin{equation}
        \frac{1,62 - 0,56}{320 - 1} = \fract{t_{hold} - 0,56}{5 - 1}
    \end{equation}
    \begin{equation}
        \frac{1,64 - 0,07}{640 - 2} = \fract{t_{NAND} - 0,07}{5 - 2} % TODO terminar
    \end{equation}
    \begin{equation}
        \frac{1,64 - 0,07}{640 - 2} = \fract{t_{NOR} - 0,07}{5 - 2} % TODO terminar
    \end{equation}
    Assim sendo, $t_{min} = 0,698 ns$ o que implica uma frequência máxima de \emph{clock} de $1,43 GHz$. \\
\bigskip
...
\end{itemize}
    ...
    \img{Coeficiente de frequência e potência do circuito.}{Q16coefFreqPotencia}{Q16coefFreqPotencia}
...
\textbf{18.} *Faça a extração do circuito via o Calibre com a opção \textbf{C+CC}. Simule o circuito com parâmetros típicos e determine a máxima velocidade e consumo do circuito. Considere as mesmas condições do exercício 16. \\
    ...
    A frequência obtida foi $0,88 GHz$ com um consumo de $1,61 mW$, conforme o gráfico da \fref{imgQ10coefFreq} demonstra. O sinal da frequência escolhida está representdo no gráfico da \fref{imgQ10maxFreq}. \\

    \img{Coeficiente de frequência. Frequência do sinal / Frequência de \emph{clock}}{imgQ18coefFreq}{imgQ18coefFreq} % XXX adicionar imagem
...
\textbf{19.} *Extraia agora com a opção \textbf{R+C+CC}. Simule o circuito com parâmetros típicos e determine a máxima velocidade e consumo do circuito. Considere as mesmas condições do exercício 16. \\
    ...
    A frequência obtida foi $X GHz$, conforme o gráfico da \fref{imgQ19coefFreq} demonstra. O sinal da frequência escolhida está representdo no gráfico da \fref{imgQ19maxFreq}. \\ % XXX colocar o valor

    \img{Coeficiente de frequência. Frequência do sinal / Frequência de \emph{clock}}{imgQ19coefFreq}{imgQ19coefFreq} % XXX adicionar imagem
...
\begin{table}[!h]
    ...
    \begin{tabular}{|l|c|c|}
        ...
        Extração R + C + CC &       0,70           &                \\  \hline % TODO
