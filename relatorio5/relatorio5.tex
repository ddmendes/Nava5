...
\section*{Introdução}
   Neste relatório são exploradas a ferramenta \emph{LeonardoSpectrum}, o roteamento de circuitos com um alto
número de componentes e a criação, manipulação e análise de circuitos gerados a partir de um código \verb:VHDL:.
...
\textbf{14.} Descreva a máquina de estados em \emph{VHDL}. Utilize o manual de \emph{VHDL} do
   ...
   A descrição da máquina de estados está descrita no \cref{srcQ14maquinaEstados}
\begin{lstlisting}[caption=Código fonte da máquina de estados, label=srcQ14maquinaEstados]
...
\textbf{16.} Feche o Leonardo e abra novamente. Leia o \emph{VHDL}, mas tome cuidado para
   ...
   Após as otimizações, foi encontrado, para o caminho crítico, um valor de atraso de $0,96 ns$.
   O esquemático da máquina de estados está representado na \fref{imgQ16schematic}

   \img{Esquemático da máquina de estados}{imgQ16schematic}
\bigskip
...
\textbf{18.} Faça as verificações com \emph{DRC} e \emph{LVS} e apresente o \emph{layout} final.
    ...
    O \emph{layout} final da máquina de estados está representado na \fref{imgQ18layout}.

    \img{\emph{Layout} da máquina de estados}{imgQ18layout}
\bigskip
...
\textbf{19.} Faça a extração do circuito com \verb:R+C+CC: e determine a máxima velocidade
   ...
   resultado com o encontrado no item 17.
    ...
    Os resultados para maior frequência de operação estão representados na \tref{tblQ19resultados}. \\
    As frequências de operação máxima da simulação foram obtidas através da \fref{imgQ19maxFreqMCbaixo} e \fref{imgQ19maxFreqMCalto}. \\

    Após uma análise dos dados, percebe-se que há uma grande diferença entre o valor calculado no \emph{LeonardoSpectrum} e os calculados na extração \verb:R+C+CC:. Isso ocorre, pois, na simulação, são considerados os elementos parasitas do circuito, que são ignorados no cálculo do \emph{LeonardoSpectrum}. \\
    \begin{table}[!h]
        \caption{Máximas frequência de operação. \label{tblQ19resultados}}
        \centering
        \begin{tabular}{|l|c|c|}
                                                                    \hline
            ...
            Simulação (MC = "1")    &        0,92               \\  \hline
            Simulação (MC = "0")    &        0,91               \\  \hline

        \end{tabular}
    \end{table}

    \img{Máxima frequência de operação para \verb:MC = "0":}{imgQ19maxFreqMCbaixo}
    \img{Máxima frequência de operação para \verb:MC = "1":}{imgQ19maxFreqMCalto}
\bigskip
