...
% TODO
\begin{lstlisting}[cation=Descrição do divisor por 8, label=srcQ02div8]
...
\textbf{3.}* Simule no \emph{ModelSim} o circuito e verifique seu funcionamento.
  ...
  \img{Simulação no \emph{ModelSim} do divisor por 8}{imgQ03simulation} % TODO adicionar imagem
\bigskip
...
do circuito é de $1,19 ns$ sua máxima frequência de operação será $0,84 GHz$.
  ...
  \img{Caminho crítico gerado pelo \emph{LeonardoSpectrum} do divisor por 8}{imgQ06criticalPath} % TODO adicionar figura
\bigskip
...
\textbf{7.}*Apresente no relatório o \emph{VHDL} e o esquemático final que obteve.
  ...
  \img{Esquemático gerado pelo \emph{LeonardoSpectrum} do divisor por 8}{imgQ07esquematico} % TODO adicionar figura
\bigskip
...
operação é $1,10 GHz$.
  ...
  \img{Esquemático gerado no \emph{ICStation} do divisor por 8}{imgQ08esquematico} % TODO adicionar figura
  \img{Máxima frequência de operação}{imgQ08maxFreq} % TODO adicionar figura
\bigskip
...
\textbf{11.} *Coloque os \emph{ports}, passe o \emph{DRC}, elimine os erros e faça
  ...
  \img{\emph{Layout} do circuito divisor por 8}{imgQ11layout} % TODO adicionar figura
\bigskip
...
operação é $0,77 GHz$.
  ...
  \img{Máxima frequência de operação (\verb:R+C+CC:)}{imgQ12maxFreq} % TODO adicionar figura
\bigskip
...
% TODO
\begin{lstlisting}[cation=Descrição do \emph{Prescaler} completo, label=srcQ14prescaler]
...
\textbf{15.} *Simule no \emph{ModelSim} o circuito e verifique seu funcionamento.
  ...
  \img{Simulação no \emph{ModelSim} do \emph{Prescaler} completo, SM = '1'}{imgQ15simulationSMalto} % TODO adicionar figura
  \img{Simulação no \emph{ModelSim} do \emph{Prescaler} completo, SM = '0'}{imgQ15simulationSMbaixo} % TODO adicionar figura
\bigskip
...
\textbf{16.} Faça a síntese e teste duas possibilidades disponíveis no Leonardo:
  ...
  \img{Esquemático do circuito, modelo 1}{imgQ16esquematico1} % TODO adicionar figura
  \img{Esquemático do circuito, modelo 2}{imgQ16esquematico2} % TODO adicionar figura
\bigskip
...
\textbf{17.} *Verifique, para o caso \emph{flat}, qual é o caminho critico (apresente
  a figura com caminho crítico e indique no esquemático completo onde ele
   está). Está correto?
    ...
    O caminho crítico está representado na \fref{imgQ17criticalPath}. Como o atraso
do circuito é de $xXX ns$ sua máxima frequência de operação será $XXX GHz$. % TODO colocar os valores

  \img{Caminho crítico gerado pelo \emph{LeonardoSpectrum} do \emph{Prescaler} completo}{imgQ17criticalPath} % TODO adicionar figura
\bigskip
...
\textbf{20.} *A partir do esquemático com hierarquia, monte o \emph{layout} do
  ...
  O \emph{layout} do circuito está representado na \fref{imgQ20layout}.

  \img{\emph{Layout} do \emph{Prescaler} completo}{imgQ20layout} % TODO adicionar figura
\bigskip
...
operação é $XXX GHz$. \\ % colocar valor
  A razão de potência consumida, em $XXX GHz$, é $XXX mW/GHz$ assim como mostra a \fref{imgQ21potXfreq}. \\ % TODO colocar valores
  As dimensões do circuito são $x \mu m$ e $y \mu m$, % (x,y) (x,y) % TODO colocar valores
o que implica em uma área total de $x \mu m^{2}$ % TODO colocar valores

  \img{Máxima frequência de operação (\verb:R+C+CC:)}{imgQ21maxFreq} % TODO adicionar figura
  \img{Razão de potência consumida por frequência}{imgQ21potXfreq} % TODO adicionar figura
\bigskip
